...
const GenerateIssuedForDyingLogs = async (details) => {
  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet("Issued For Dying Logs");
  ...
  details.forEach((log) => {
    const row = worksheet.addRow({
      ...
      issued_dying_quantity:
        log.data.fullDocument.issued_dying_quantity.total,
...
export { GenerateIssuedForDyingLogs };
