...
const GenerateDyedGroupsReport = async (details) => {

  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet("Dyed Groups Reports");
  ...
  worksheet.columns = [
    ...
    { header: "Remark", key: "group_dying_remarks", width: 20 },
  ...
  details.forEach((order) => {
    order.item_details.forEach((Item) => {
      const row = worksheet.addRow({
        ...
        group_dying_remarks: order.group_dying_remarks,
  ...
  const filePath =
    "public/reports/Dying/DyedGroupsReportExcel/dyed_groups_reports.xlsx";
  ...
  const downloadFileName = `dyed_groups_reports${timestamp}.xlsx`;

  // Move the file to the desired folder
  const destinationPath = `public/reports/Dying/DyedGroupsReportExcel/${downloadFileName}`;
...
export { GenerateDyedGroupsReport };
