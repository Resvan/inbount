...
const GenerateIssueForSmokingReport = async (details) => {

  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet("Raw issue For Smoking Reports");
  // Add headers to the worksheet
  const headers = [
    { header: "Issued Date", key: "issued_data", width: 15 },
    ...
    {
      header: "Issued Smoking Quantity Total",
      key: "Issued_total",
      width: 20,
    },

    {
      header: "Received Quantity Natural	",
      key: "Received_natural",
      width: 20,
    },
    ...
    {
      header: "Available Quantity Smoked",
      key: "Available_smoked",
      width: 20,
    },
    { header: "Available Quantity Total", key: "Available_total", width: 20 },

    {
      header: "Rejected Quantity Natural	",
      key: "Rejected_natural",
      width: 20,
    },
  ...
  details.forEach((order) => {
    ...
    const row = worksheet.addRow({
      issued_data: convDate(order.created_at),
      ...
      Issued_total: order.issued_smoking_quantity.total,
  ...
  const filePath =
    "public/reports/Raw/issueForSmokingReportExcel/IssuedForSmokingRawReport.xlsx";
  ...
  const downloadFileName = `Issued_For_Smoking_Raw_Report${timestamp}.xlsx`;

  // Move the file to the desired folder
  const destinationPath = `public/reports/Raw/issueForSmokingReportExcel/${downloadFileName}`;
...
export { GenerateIssueForSmokingReport };
