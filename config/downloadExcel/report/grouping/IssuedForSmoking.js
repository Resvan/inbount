...
const GenerateIssuedForSmokingGroupsReport = async (details) => {

  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet("Issued For Smoking Groups Reports");
  ...
  details.forEach((order) => {
    order.group_id.item_details.forEach((Item) => {
  ...
  const filePath =
    "public/reports/NewRport/IssuedForSmokingGroupsReportExcel/issue_for_smoking_groups_reports.xlsx";
  ...
  const downloadFileName = `issue_for_smoking_groups_reports${timestamp}.xlsx`;

  // Move the file to the desired folder
  const destinationPath = `public/reports/NewRport/IssuedForSmokingGroupsReportExcel/${downloadFileName}`;
...
export { GenerateIssuedForSmokingGroupsReport };
